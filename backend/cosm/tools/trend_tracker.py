        ...
        for keyword in keywords[:3]:  # Limit to prevent rate limiting
            ...
            for query in trend_queries:
                try:
                    ...
                    trend_insights = extract_trend_insights_with_gemini(search_results, keyword)
                    
                    if trend_insights:
                        trend_data["search_volume_indicators"].extend(trend_insights.get("volume_indicators", []))
                        trend_data["related_trends"].extend(trend_insights.get("related_trends", []))
                        trend_data["emerging_subtopics"].extend(trend_insights.get("subtopics", []))
...
def track_industry_momentum(industry: str, keywords: List[str]) -> Dict[str, Any]:
    ...
    try:
        ...
        for query in momentum_queries:
            try:
                search_results = search_web_real(query, max_results=3)
                momentum_insights = extract_momentum_insights_with_gemini(search_results, industry)
                
                if momentum_insights:
                    momentum_data["growth_indicators"].extend(momentum_insights.get("growth_indicators", []))
                    momentum_data["innovation_signals"].extend(momentum_insights.get("innovation_signals", []))
                    momentum_data["investment_trends"].extend(momentum_insights.get("investment_trends", []))
                    momentum_data["disruption_signals"].extend(momentum_insights.get("disruption_signals", []))
...
def extract_trend_insights_with_gemini(search_results: List[Dict[str, str]], keyword: str) -> Dict[str, Any]:
...
def extract_momentum_insights_with_gemini(search_results: List[Dict[str, str]], industry: str) -> Dict[str, Any]:
