...
def monitor_social_sentiment(
    ...
    try:
        ...
        for platform in platforms:
            platform_data = _analyze_platform_sentiment(
                topic, platform, timeframe_days, min_engagement
            )
            sentiment_data["platform_breakdown"][platform] = platform_data

            # Weight platform sentiment by volume and credibility
            platform_weight = platform_data["volume"] * platform_data.get(
        ...
        if total_weight > 0:
            ...
            sentiment_data["overall_sentiment"] = _classify_sentiment(
                sentiment_data["sentiment_score"]
            )

        # Extract cross-platform insights
        sentiment_data["key_themes"] = _extract_key_themes(
            sentiment_data["platform_breakdown"]
        )
        sentiment_data["trending_discussions"] = _find_trending_discussions(
            sentiment_data["platform_breakdown"]
        )
        sentiment_data["sentiment_drivers"] = _identify_sentiment_drivers(
            sentiment_data["platform_breakdown"]
        )
...
def _analyze_platform_sentiment(
    topic: str, platform: str, timeframe_days: int, min_engagement: int
) -> Dict[str, Any]:
    ...
    try:
        ...
        if platform == "reddit":
            ...
            platform_data = _analyze_generic_platform_sentiment(
                topic, platform, timeframe_days
            )
...
def _analyze_reddit_sentiment(topic: str, timeframe_days: int) -> Dict[str, Any]:
    ...
    return {
        ...
        "top_posts": [
            ...
            {
                "title": f"Frustrated with {topic} limitations",
                "score": 67,
                "comments": 45,
            },
        ],
...
def _analyze_twitter_sentiment(topic: str, timeframe_days: int) -> Dict[str, Any]:
    ...
    return {
        ...
        "top_posts": [
            {
                "content": f"Really need a better solution for {topic}",
                "likes": 34,
                "retweets": 12,
            },
            {
                "content": f"Why doesn't {topic} work the way I expect?",
                "likes": 23,
                "retweets": 8,
            },
        ],
...
def _analyze_linkedin_sentiment(topic: str, timeframe_days: int) -> Dict[str, Any]:
    ...
    return {
        ...
        "top_posts": [
            {
                "content": f"Industry insights on {topic} transformation",
                "likes": 67,
                "comments": 23,
            },
            {
                "content": f"Best practices for {topic} implementation",
                "likes": 45,
                "comments": 12,
            },
        ],
...
def _analyze_generic_platform_sentiment(
    topic: str, platform: str, timeframe_days: int
) -> Dict[str, Any]:
...
def _extract_key_themes(platform_breakdown: Dict[str, Any]) -> List[Dict[str, Any]]:
    ...
    return [
        ...
        for theme, freq in Counter(themes).most_common(10)
    ]


def _find_trending_discussions(
...
def _generate_sentiment_alerts(sentiment_data: Dict[str, Any]) -> List[Dict[str, str]]:
    ...
    # Volume-based alerts
    total_volume = sum(
        data.get("volume", 0) for data in sentiment_data["platform_breakdown"].values()
    )
    if total_volume > 200:
        alerts.append(
            {
...
def process_social_signals(signals: List[Dict[str, Any]]) -> List[Dict[str, Any]]:
    ...
    try:
        ...
        processed_signals["confidence_scores"] = _calculate_confidence_scores(
            processed_signals
        )
...
def _process_individual_signal(signal: Dict[str, Any]) -> Dict[str, Any]:
    ...
    try:
        ...
        # Analyze sentiment
        processed["sentiment"] = _analyze_content_sentiment(
...
def _assess_urgency(content: str) -> str:
    ...
    urgent_count = sum(
        1 for indicator in urgent_indicators if indicator in content_lower
    )
    moderate_count = sum(
        1 for indicator in moderate_indicators if indicator in content_lower
    )
...
def _categorize_signal(signal: Dict[str, Any]) -> str:
    ...
    keywords: List[str] = signal.get("keywords", [])  # noqa: F841
    sentiment = signal.get("sentiment", "neutral")

    # Feature request indicators
    if any(
        word in content
        for word in ["need", "want", "wish", "should add", "feature request"]
    ):
        return "feature_request"

    # Pain point indicators
    if sentiment == "negative" and any(
        word in content for word in ["problem", "issue", "broken", "doesn't work"]
    ):
        return "pain_point"

    # Complaint indicators
    if sentiment == "negative" and any(
        word in content for word in ["frustrated", "hate", "terrible", "awful"]
    ):
        return "complaint"

    # Suggestion indicators
    if any(
        word in content for word in ["suggest", "recommend", "could", "might", "better"]
    ):
        return "suggestion"
...
def _summarize_categories(processed_signals: Dict[str, Any]) -> Dict[str, Any]:
    ...
    return {
        ...
        "distribution": {
            "pain_points": len(processed_signals["pain_points"])
            / max(processed_signals["total_signals"], 1),
            "feature_requests": len(processed_signals["feature_requests"])
            / max(processed_signals["total_signals"], 1),
            "complaints": len(processed_signals["complaints"])
            / max(processed_signals["total_signals"], 1),
...
def _calculate_confidence_scores(processed_signals: Dict[str, Any]) -> Dict[str, float]:
    ...
    specificity_confidence = min(
        high_specificity_count / max(total_signals * 0.3, 1), 1.0
    )
    ...
    return {
        ...
        "overall_confidence": (
            volume_confidence + diversity_confidence + specificity_confidence
        )
        / 3,
    }
...
def identify_patterns(signals: List[Dict[str, Any]], context: str) -> Dict[str, Any]:
    ...
    try:
        ...
        pattern_analysis["patterns"]["workflow_patterns"] = _identify_workflow_patterns(
            processed_signals
        )

        # Identify integration patterns
        pattern_analysis["patterns"]["integration_patterns"] = (
            _identify_integration_patterns(processed_signals)
        )

        # Identify user journey patterns
        pattern_analysis["patterns"]["user_journey_patterns"] = (
            _identify_user_journey_patterns(processed_signals)
        )

        # Identify pain point patterns
        pattern_analysis["patterns"]["pain_point_patterns"] = (
            _identify_pain_point_patterns(processed_signals)
        )

        # Identify solution patterns
        pattern_analysis["patterns"]["solution_patterns"] = _identify_solution_patterns(
            processed_signals
        )

        # Find connections between patterns
        pattern_analysis["connections"] = _find_pattern_connections(
            pattern_analysis["patterns"]
        )

        # Identify trend indicators
        pattern_analysis["trend_indicators"] = _identify_trend_indicators(
            processed_signals, context
        )

        # Extract opportunity signals
        pattern_analysis["opportunity_signals"] = _extract_opportunity_signals(
            pattern_analysis
        )

        # Calculate pattern confidence
        pattern_analysis["pattern_confidence"] = _calculate_pattern_confidence(
            pattern_analysis
        )
...
def _identify_integration_patterns(
    processed_signals: Dict[str, Any],
) -> List[Dict[str, Any]]:
    ...
    all_signals = processed_signals.get("pain_points", []) + processed_signals.get(
        "feature_requests", []
    )
...
def _identify_user_journey_patterns(
    processed_signals: Dict[str, Any],
) -> List[Dict[str, Any]]:
    """Identify user journey patterns"""
    journey_indicators = [
    ...
    for signal in processed_signals.get("complaints", []):
        ...
        journey_matches = [
            indicator for indicator in journey_indicators if indicator in content
        ]
...
def _identify_journey_stage(content: str) -> str:
    ...
    if any(
        word in content
        for word in ["setup", "install", "getting started", "onboarding"]
    ):
        return "onboarding"
...
def _identify_pain_point_patterns(
    processed_signals: Dict[str, Any],
) -> List[Dict[str, Any]]:
...
def _identify_solution_patterns(
    processed_signals: Dict[str, Any],
) -> List[Dict[str, Any]]:
    """Identify solution request patterns"""
    solution_types = defaultdict(list)

    all_requests = processed_signals.get(
        "feature_requests", []
            ...
            patterns.append(
                {
                    ...
                    "specificity_distribution": Counter(
                        s.get("specificity") for s in signals
                    ),
        ...
        connections.append(
            {
                ...
                "strength": "strong"
                if len(workflow_patterns) >= 3 and len(integration_patterns) >= 3
                else "moderate",
            }
        )
...
def _identify_trend_indicators(
    processed_signals: Dict[str, Any], context: str
) -> List[Dict[str, Any]]:
...
def _extract_opportunity_signals(
