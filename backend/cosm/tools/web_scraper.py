...
def scrape_reddit_discussions(html_content: str, topic: str) -> List[Dict[str, Any]]:
    ...
    try:
        ...
        post_titles = soup.find_all(
            ["h1", "h2", "h3"], class_=re.compile(r"title|heading|post")
        )
        comments = soup.find_all(
            ["div", "p"], class_=re.compile(r"comment|text|content")
        )
        ...
        for comment_elem in comments[:20]:  # Top 20 comments
            ...
            if (
                len(comment_text) > 50 and len(comment_text) < 500
...
def scrape_twitter_complaints(html_content: str, topic: str) -> List[Dict[str, Any]]:
    ...
    try:
        ...
        tweets = soup.find_all(
            ["div", "p", "span"], class_=re.compile(r"tweet|text|content")
        )
...
def extract_pain_points(html_content: str, topic: str) -> List[Dict[str, Any]]:
    ...
    try:
        ...
        content_elements = soup.find_all(
            ["div", "p", "li"], class_=re.compile(r"question|answer|post|content")
        )
        ...
        for elem in content_elements[:25]:  # Top 25 elements
            ...
            if len(text) > 40 and len(text) < 800:  # Reasonable content length
                ...
                if topic.lower() in text_lower and any(
                    indicator in text_lower for indicator in pain_indicators
                ):
                    signals.append(
                        {
...
def monitor_social_sentiment(topic: str, timeframe_days: int = 7) -> Dict[str, Any]:
    ...
    try:
        ...
        all_sentiments = [
            data["sentiment_score"] for data in sentiment_data["platforms"].values()
        ]
        sentiment_data["overall_sentiment"] = calculate_overall_sentiment(
            all_sentiments
        )
        sentiment_data["frustration_level"] = calculate_frustration_level(
            sentiment_data["platforms"]
        )
    ...
    for platform_data in platforms_data.values():
        if platform_data.get("frustration_indicators", 0) > 0:
            frustration_scores.append(
                platform_data["frustration_indicators"] / 20.0
            )  # Normalize
