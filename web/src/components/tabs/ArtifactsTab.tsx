...
export function ArtifactsTab({ appName, userId, sessionId }: ArtifactsTabProps) {
  ...
  const deleteArtifact = async (artifactName: string) => {
    try {
      await api.delete(`/apps/${appName}/users/${userId}/sessions/${sessionId}/artifacts/${artifactName}`)
      fetchArtifacts()
    } catch (error) {
        ...
        {artifacts.map((artifact) => (
          <Card key={artifact.name} className="bg-gray-800 border-gray-700 p-4">
            <div className="flex items-center justify-between mb-2">
              <h3 className="text-lg font-medium text-gray-100">{artifact.name}</h3>
